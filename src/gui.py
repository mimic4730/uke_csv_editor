...
class UKEEditorGUI(tk.Tk):
    ...
    def _insert_help_text(self):
        """起動時にテキストボックスへ簡易ヘルプを表示（v1.6.3対応）"""
        HELP_TEXT = (
            f"UKE CSV Editor {APP_VERSION} - 使い方ガイド\n"
            "============================================\n"
            "■ 重要：ハイライト用桁数（N）について\n"
            "  - ハイライト用桁数は『枝番を除いた基準桁（N）』です。\n"
            "  - 後方カンマモードでは、検出時に {N, N+枝番桁} を許容します（例：N=10・枝番2桁→{10,12}）。\n"
            "  - 枝番の赤塗り／枝番除去／フォールバックは、長さが N+枝番桁 のときだけ適用されます。\n"
            "\n"
            "■ 基本の流れ\n"
            ...
            "  - 第2段（フォールバック）: 第1段で不変の場合、長さが N+枝番桁 のコードに限り\n"
            "      末尾の枝番桁数を落としてから桁揃えします。\n"
            "\n"
            "■ 設定\n"
            "  - 患者コード桁数（ハイライト用）: 枝番を除いた基準桁 N。\n"
            "      ・後方カンマ数モード : 検出は {N, N+枝番桁} を許容。\n"
            "      ・任意の記号モード   : 1～N 桁（例: 12603-0002 → 12603 を検出）。\n"
            "  - 患者コード変換桁数     : 保存時の桁数。長い→左カット／短い→左0埋め。\n"
            "  - 後方カンマ数           : コード直後に続く空フィールド数（入力フォーマットに合わせる）。\n"
            "  - 枝番処理                : オフ/1桁/2桁 を選択（登録済み枝番を参照）。\n"
            "\n"
            "■ ヒント\n"
            "  - まず『ファイル読み込み』→『患者コード・全行ハイライト』で検出状況を確認。\n"
            "  - フォールバック件数は完了ダイアログと _changes.csv の method=fallback で確認できます。\n"
            "  - N=10・枝番2桁（例）では、10桁は枝番なし／12桁は枝番ありとして扱います。\n"
        )
        self.row_text.config(state="normal")
        self.row_text.delete("1.0", "end")
